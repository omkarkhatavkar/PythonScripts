	...
	for key in sorted(clusterName):
		...
		if key.find(terminalENV)!=-1:
			try:
				...
				cmd="sshpass -p 'Kn0od!e' ssh -o StrictHostKeyChecking=no -t  deceapp@{0} grep -n {1} /opt/jetty_coordinator/jetty/logs/dece* | cut -d : -f 2 | head -1 ".format(machineName,grepString)
