...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Testing@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} unzip -c /opt/jetty_domain_manager/jetty/wars/decedm.war META-INF/MANIFEST.MF | grep Implementation-Version'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
        output = cluster_value.communicate()
        dictionary[key] = output[0]

f = open('/var/www/html/getDOMAINVersion.html','w')
