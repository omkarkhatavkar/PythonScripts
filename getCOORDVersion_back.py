...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Network@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} unzip -c /opt/jetty_coordinator/jetty/wars/dece-1.11.war  META-INF/MANIFEST.MF | grep Implementation-Version:'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
