...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Network@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} grep db.url /opt/java7_DECE_batch/config/batchProcess.properties'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)

        output = cluster_value.communicate()
        dictionary[key] = output[0]
	print output[0]
f = open('/var/www/html/getBATCHDBDetails.html','w')
