...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "qauv13" ssh -o StrictHostKeyChecking=no -t QAUV@{0} pkill -f testrunner'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
	output = cluster_value.communicate()
        cluster_value = subprocess.Popen('sshpass -p "qauv13" ssh -o StrictHostKeyChecking=no -t QAUV@{0} ps -ef| grep "testrunner.sh" | grep -v grep'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
