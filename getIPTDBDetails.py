...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Network@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} grep db.url /opt/java7_DECE_ipt/config/iptConfig.properties'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
