...
for key in sorted(clusterName):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Network@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} unzip -p /opt/java7_DECE_ipt/dece-ipt.jar META-INF/MANIFEST.MF | grep Implementation-Version'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
        output = cluster_value.communicate()
        dictionary[key] = output[0].split(':')[-1]

f = open('/var/www/html/getIPTVersion.html','w')
