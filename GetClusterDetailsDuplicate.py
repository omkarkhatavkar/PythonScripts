...
for index in range(len(clusterName)):
        ...
        cluster_value = subprocess.Popen('sshpass -p "Network@28" ssh -o StrictHostKeyChecking=no -t okhatav@{0} grep clu1 /opt/jetty_coordinator/jetty/conf/dece.properties'.format(machineName), stdout=subprocess.PIPE, stderr=None, shell=True)
        output = cluster_value.communicate()
	dictionary[index] = output[0]
...
<p id="demo">&nbsp;</p>
<script>
document.getElementById("demo").innerHTML ="Last Updated On : "+ Date();
</script>
